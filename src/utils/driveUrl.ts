 ...
 */
export function formatImageUrl(url: string, driveWidth: number = 600): string {
  if (!url) return '';
  ...
  if (trimmed.includes('drive.google.com/thumbnail')) {
    if (trimmed.includes('sz=')) {
      return trimmed.replace(/sz=[a-zA-Z0-9_-]+/, `sz=w${driveWidth}`);
    }
    return `${trimmed}&sz=w${driveWidth}`;
  }
  ...
  if (driveFileMatch && driveFileMatch[1]) {
    return `https://drive.google.com/thumbnail?id=${driveFileMatch[1]}&sz=w${driveWidth}`;
  }
  ...
  if (driveIdMatch && driveIdMatch[1] && trimmed.includes('drive.google.com')) {
    return `https://drive.google.com/thumbnail?id=${driveIdMatch[1]}&sz=w${driveWidth}`;
  }
