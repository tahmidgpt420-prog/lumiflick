 ...
 */
export function formatImageUrl(url: string, driveWidth: number | 'original' = 240): string {
  if (!url) return '';
  ...
  const szParam =
    driveWidth === 0 || driveWidth === 'original'
      ? 's0'
      : typeof driveWidth === 'string' && (driveWidth as string).includes('-rw')
      ? `w${driveWidth}`
      : `w${driveWidth}-rw`;
